...
def inter_frame(frame):
    ...
    # Send frame for prediction using Roboflow API
    response = CLIENT.infer(frame, model_id=MODEL_ID)

    # Return prediction as dict
    if response:
        return response
    else:
...
def main ():
    ...
    # Prepare video writer with same resolution and fps as input video #
    ...
    while True:
        ...
        if predictions and "predictions" in predictions:
            for pred in predictions["predictions"]:
                ...
                # Extract bounding box center cordinates and size
                x, y = int(pred["x"]), int(pred["y"])
                w, h = int(pred["width"]), int(pred["height"])
                class_name = pred["class"].lower()

                # Display labels as detected 
